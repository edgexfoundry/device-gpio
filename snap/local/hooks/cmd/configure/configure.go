...
func main() {
	...
	if err != nil {
		fmt.Println(fmt.Sprintf("edgex-device-gpio:configure: can't read value of 'debug': %v", err))
		os.Exit(1)
	}
	if status == "true" {
		debug = true
	...
	if err = hooks.Init(debug, "edgex-device-gpio"); err != nil {
		fmt.Println(fmt.Sprintf("edgex-device-gpio:configure: initialization failure: %v", err))
		os.Exit(1)
	...
	if envJSON != "" {
		hooks.Debug(fmt.Sprintf("edgex-device-gpio:configure: envJSON: %s", envJSON))
		err = hooks.HandleEdgeXConfig("device-gpio", envJSON, local.ConfToEnv)
		if err != nil {
	...
	if autostart == "" {
		hooks.Debug("edgex-device-gpio autostart is NOT set, initializing to 'no'")
		autostart = "no"
	}
	autostart = strings.ToLower(autostart)

	hooks.Debug(fmt.Sprintf("edgex-device-gpio autostart is %s", autostart))
	...
	case "true":
		fallthrough
	case "yes":
		err = cli.Start("device-gpio", true)
		if err != nil {
	...
	if err := options.ProcessAppConfig("device-gpio"); err != nil {
